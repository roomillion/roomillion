...
工作台现在内置七个示例：物资台账、会议行动项、离线 3D 晶体挑战、AI 辩论场、AI模型能力测试、千万间浏览器和文档浏览与转换。点击左侧“示例房间”即可查看用途与权限后安装；除“千万间浏览器”外均保持普通互联网离线，“千万间浏览器”只有在用户授权并打开工作台联网总开关后才访问网页。文档房间可离线浏览 Markdown、DOCX、XLS/XLSX、PPTX、PDF，并在 Markdown、Word、PDF 间互转。它们覆盖数据库、文件导入导出、AI 简报、Three.js/Rapier 3D 游戏、主工作台多模型编排、多模态评测、受控网页浏览和最小权限等不同组合。详细说明见 [17-内置示例房间库](docs/17-内置示例房间库.md)，浏览器能力边界见 [37-room-browser-v1规范与浏览器房间](docs/37-room-browser-v1规范与浏览器房间.md)。

公开仓库和发行构建只包含上述七个公开示例；未公开示例不在源码或打包清单中。发行配置使用明确文件名，不会自动收录资源目录中的其他 `.room` 文件。
