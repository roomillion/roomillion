...
当前 Windows 源码版本为 **0.4.0**，generic Linux x64 技术预览仍为 **0.3.0-alpha.36**。Windows 普通用户推荐安装版；安装到当前用户、不要求管理员权限，并注册 `.room` 文件关联。仍提供目录 ZIP 和单文件免安装版。构建产物采用统一命名：
...
三种 Windows 交付都不要求目标电脑预装 Node、Git、SQLite 或 npm 包。安装版支持直接双击 `.room`，但仍显示解密、完整性、权限和风险预检，用户确认后才安装；便携版继续通过导入或拖入使用。**两种便携版的数据位置不同**：解压 ZIP 版默认把房间和工作台数据放在解压目录的 `Roomillion-data/`；单文件便携 EXE 默认使用当前用户的数据目录（通常为 `%APPDATA%\Roomillion\mvp-data`），首次运行可自定义到其他目录，自定义位置下创建 `Roomillion-data/`。安装版继续使用当前用户的数据目录。可在“设置 → 房间位置”查看和更改，迁移会在重启后复制数据，保留旧目录；再次导入房间时文件对话框会从上次导入目录打开。复制单文件 EXE 本身不会带走房间；跨电脑迁移仍可使用“房间更多 → 导出 → 应用＋数据”。API Key 由系统加密，换电脑后需重新配置。`.room` 统一支持仅应用/应用＋数据，以及可选密码。旧版 `.zroom` 仍可导入和双击打开，新导出统一使用 `.room`。房间还可内带图标；AI 默认生成图标，也可按用户明确要求采用最近上传的图片，规则见 [安装版与房间图标说明](docs/49-Windows安装版文件关联与房间图标.md)。

关闭工作台窗口时可选择“常驻后台”“完全退出”或“取消”。常驻后台会隐藏工作台与当前可见的独立窗口，但不会销毁房间、AI 任务或数据库连接；点击系统托盘中的千万间图标可恢复，托盘菜单也提供“完全退出”。
...
工作台现在内置八个示例：物资台账、会议行动项、离线 3D 晶体挑战、AI 辩论场、AI模型能力测试、千万间浏览器、文档浏览与转换和音频分析室。点击左侧“示例房间”即可查看用途与权限后安装；除“千万间浏览器”外均保持普通互联网离线，“千万间浏览器”只有在用户授权并打开工作台联网总开关后才访问网页。文档房间可离线浏览 Markdown、DOCX、XLS/XLSX、PPTX、PDF，并在 Markdown、Word、PDF 间互转。音频分析室可把音乐或会议录音交给声明了音频能力的模型，分析乐器与风格或转写成 Markdown。它们覆盖数据库、文件导入导出、AI 简报、Three.js/Rapier 3D 游戏、主工作台多模型编排、多模态评测、音频 AI、受控网页浏览和最小权限等不同组合。详细说明见 [17-内置示例房间库](docs/17-内置示例房间库.md)，浏览器能力边界见 [37-room-browser-v1规范与浏览器房间](docs/37-room-browser-v1规范与浏览器房间.md)。

公开仓库和发行构建只包含上述八个公开示例；未公开示例不在源码或打包清单中。发行配置使用明确文件名，不会自动收录资源目录中的其他 `.room` 文件。
...
AI 能力设置除生成式 LLM 外，还可独立配置 Embedding、Rerank 和“直觉”模型。房间通过统一网关调用 `room.ai.embed()`、`room.ai.rerank()` 与 `room.ai.intuition()`，API Key 和服务地址不会暴露给房间；“直觉”当前接入 TypeSafe AI 的 Jev/System One 有类型概率决策接口。详见 [55-专用AI模型与Jev直觉接口](docs/55-专用AI模型与Jev直觉接口.md)。
...
- 文档版本：`0.1-mvp`
- 当前阶段：Windows x64 源码版本 `0.4.0`，generic Linux x64 `0.3.0-alpha.36` 技术预览；指定 UOS 实机认证和 macOS 运行时仍待执行
- 产品名称：千万间 Roomillion
