...
公开仓库和发行构建只包含上述六个公开示例；未公开示例不在源码或打包清单中。发行配置使用明确文件名，不会自动收录资源目录中的其他 `.room` 文件。
