...
- `docs/` — local-only design and acceptance records, excluded from the public repository. Consult relevant local notes when available; public code and tests must remain understandable without them.
- `examples/` — sources for the eight public example rooms; packed into `resources/examples/*.room` by build scripts.
...
- All process code is CommonJS `.cjs` with `"use strict"` and `node:`-prefixed builtins. Do not introduce TypeScript or a bundler without checking the existing architecture first.
...
- Room packaging is "shared layer first": deps provided by the workbench are referenced, not copied; out-of-catalog deps must ship in the room's `embedded/` with exact versions and licenses.
- API keys are system-encrypted and never exposed to rooms; AI access goes through the AI gateway and respects the workbench network switch.
...
Review the relevant implementation and tests before changing room packages, the capability broker, the Agent harness, dependency packing, or release scripts. Local `docs/` notes may provide history, but are not part of the public source checkout. Keep the README, tests, and public compliance records accurate without linking to private notes.
