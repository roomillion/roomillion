      ...
      for (const action of scenario.actions) {
        ...
        await webContents.executeJavaScript(`(async () => {
          ...
          if (action.type === 'assertText') {
            ...
            if (!actual.includes(action.value)) throw new Error('测试场景“' + scenarioName + '”文本断言失败：' + action.selector + '；期望包含 ' + JSON.stringify(action.value.slice(0, 120)) + '，实际为 ' + JSON.stringify(actual.slice(0, 300)));
          }
