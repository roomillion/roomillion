...
`room.ai.generate(prompt, options)` 返回 `{ text, model, profileId, usage }`。`room.ai.batch(requests, { concurrency })` 返回 `{ results, total, passed, failed }`；成功项直接是 `{ ok: true, text, model, profileId, usage }`，失败项是 `{ ok: false, error }`，不要读取 `result.value.text`。视觉输入可传 `{ data: Uint8Array, mimeType }`、`{ blobId }` 或 `{ directory: { grantId, relativePath } }`。
...
批量文件或视觉 AI 房间必须增加 `room-tests.json`。用 1–20 个简短场景覆盖关键按钮和结果，动作支持 `click`、`input`、`wait`、`assertExists`、`assertText`；`mocks.ai` 可提供隔离测试所需的模型响应，不调用真实 AI。
