...
- [第三方组件目录](THIRD-PARTY-NOTICES.md)：核心运行依赖、39 个官方房间模块及工具链用途、版本与许可。
- [逐包许可原文索引](THIRD-PARTY-LICENSES.md)：运行依赖及房间资源来源/传递依赖的原始 LICENSE、COPYING、NOTICE、版权文件及 SHA-256。
- [完整组件清单（SBOM）](sbom.cdx.json)：构建所用依赖版本和来源，不代替许可证文本。
- [待核对事项](LICENSE-REVIEW.md)：缺少独立许可原文、人工来源判断、工具链源码提供等事项。不得将清单存在解释为所有公开分发义务均已履行。
